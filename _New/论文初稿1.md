...
最后，如下图中所示，产生了正确的颜色混合效果，颜色混合的区域的大小还可以通过系数进行调整。但是颜色混合区域存在面数过多的问题，相邻正六边形颜色混合区域是由4个三角面片组成的，三种颜色的混合区域中三角面片还会更多，三角面片的数量会在很大程度上影响程序的运行效率，当地图由几万甚至几十万个正六边形地图块组成时，这些颜色混合区域很可能造成游戏运行的卡顿、掉帧等情况。
