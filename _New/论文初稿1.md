...
但是这种计算方式，最终在四舍五入的时候，可能会产生X、Y、Z之和部位0的情况，也就是在四舍五入时会产生误差。而且，鼠标点击位置距离一个正六边形地图块的重点越远，其四舍五入的误差越大，最终解决的方案是，舍弃具有最大舍入增量的坐标，并从其他两个坐标中重构它。这样，当鼠标点击的时候，就可以显示出当前点击的正六边形地图块的坐标了。代码如下：
