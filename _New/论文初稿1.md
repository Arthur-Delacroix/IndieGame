...
#### 27. 定义不同的连接类型
现在两个相邻的正六边形地图块之间已经生成了“过渡台阶”，但是仔细观察可以发现，即使高差相同的两个正六边形地图块之间的连接区域，也会生成“过渡台阶”，虽然在视觉效果上与之前没没有差别，但是产生了很多三角面，对硬件性能是一种浪费。其次，在相邻且高差大于1个单位的正六边形地图块之间，生成的“过渡台阶”视觉效果并不美观，这种情况下应该保持原有的平面类型，而不是台阶类型。所以，此处声明一个美剧，用来定义不同的地形，代码如下：
...
如果L与R位置的正六边形地图块高度相同，并且都比B位置的正六边形地图块高1单位，将这种情况识别为“slope-slope-flat”（简称“SSF”）。示意图如下：
...
这样，当地形中某三个相邻的正六边形地图块符合SSF这种情况的时候，它们之间的三角形连接区域就会变成空的，效果如下图：
...
根据SFS和FSS这两种情况，需要在TriangulateCorner方法中添加相应的判断，来生成它们之间的三角形连接区域。代码与运行效果图如下：
...
至此，SSF、SFS、FSS三种情况下，三角形连接区域都已经正确的生成了三角形连接区域。剩余部分就是高差大于1个单位的各类情况。这种情况相较于高差为1个单位的情况更加复杂，下面会进行分类处理。
