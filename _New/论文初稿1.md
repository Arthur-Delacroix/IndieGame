...
在地形系统中，除了通过颜色表示地形的种类，还需要有高度来模拟高地、悬崖和盆地等地貌。HexMap模拟地形的高度思路，是单独为每一个正六边形地图块赋予单独的海拔高度，在HexCell.cs中添加如下代码：
...
#### 33. 连接区域底部的三角剖分
如上一章的图中所示，三角形连接区域的一条边要与“过渡台阶”的边缘相匹配。单独将底部看作一个三角形区域的话，该部分的三角剖分应如下图所示，左侧边是与“过渡台阶”相匹配的边。

![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/collapsing-terraces.png)

三角形右侧的顶点，是L平面的高度，确定该顶点高度后，使用插值便可以得出该顶点在三角形连接区域中的具体位置和颜色值。这里首先确定该位置，先生成一个没有经过三角剖分的区域。代码与运行效果图如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/lower-triangle.png)

接下来就是对该区域惊醒三角剖分，这里的思路与生成“过渡台阶”的思路类似，首先构建第一个底部的三角形区域，代码与运行效果如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/first-step.png)

接下来，是构建最后一个三角形，代码与运行效果如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/last-step.png)


最后，在两者之间的所有三角形，通过循环生成，代码与运行效果图如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/all-steps.png)

#### 34. 连接区域顶部的三角剖分
底部完成后，接下来要对顶部进行三角剖分。顶部有两种情况，一种是相邻的两个连接区域均是平面，另一种是一侧是平面，另一侧则是“过渡台阶”。这里，需要修改TriangulateCornerTerracesCliff方法，将进行三角剖分的代码单独提取出来，而自身之进行组合的判断，代码如下：
...
剩余部分则相对简单一些，首先判断相邻的连接区域是否为“过渡台阶”，如果是过渡台阶，则再次调用TriangulateBoundaryTriangle方法，如果不是，则一个完整的三角形搞好可以填补该区域，代码与运行效果图如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-slope.png)
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/single-slope.png)

#### 35. 镜像位置的处理
目前已经完成了SCS与SCC组合的一侧连接区域的三角剖分，接下来需要完成其另一侧的三角剖分，其组合示意图如下：

![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/css-csc.png)

创建TriangulateCornerCliffTerraces方法，该方法与TriangulateCornerTerracesCliff方法类似，只是在处理顶点的位置时做了镜像。代码如下：
...
最后，将该方法在TriangulateCorner中进行调用，确保其可以正确的构建连接区域的三角形。代码与运行效果图如下：
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-slope-mirrored.png)
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/single-slope-mirrored.png)


#### 36. 双悬崖组合
接下来完成最后两种组合，即L和R均比B高出2单位，并且L和R高度差1。如下图所示：

![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-cliff-slope-right.png)
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-cliff-slope-left.png)

这两种组合，称为CCSR和CCSL组合，如下图所示：

![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/ccsr-ccsl.png)

这两种组合的处理方式，可以通过TriangulateCornerCliffTerraces和TriangulateCornerTerracesCliff方法进行三角剖分，在TriangulateCorner方法中通过判断L和R的高度来区分两种情况，代码如下：
...
你大师这里要注意一点，虽然这里也是将连接区域分为两个部分进行三角剖分，但是这里这里插值计算可能为负数，这样会使其产生错误的三角剖分，修改TriangulateCornerTerracesCliff方法，使其插值适中为大于零的值。代码与运行效果图如下：
...
```
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-cliff-slope-right-filled.png)
![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/double-cliff-slope-left-filled.png)

#### 37. 整理代码逻辑
代码已经覆盖了所有的组合，如下图。至此，地图系统基本完成。不过，通过代码检查，在TriangulateCorner方法中，可以通过else块来代替某些return语句，让整体的逻辑更加清晰，整理后的代码如下：

![](https://catlikecoding.com/unity/tutorials/hex-map/part-3/merging-slopes-and-cliffs/diagram.png)
...
#### 38. 结论
本文通过Unity3D引擎，开发了一款基本的地图系统。该地图系统对Unity3D版本的依赖性较低，并且完全可以数据化存储，最大程度的缩小了美术资源占体积，而且可以通过程序控制地图颜色、高低、尺寸等数据，实现完全随机化的模拟各类地貌的功能，使游戏的可玩性增加，减少重复感，提升用户体验。
该地图系统还可以通过读取灰度图来随机生成不同地貌的地形，例如，程序人员只需要一张128x128尺寸的随机灰度贴图，通过读取该贴图内的灰度值，转换为地形高度，就可以随机生成多种不同类型的地形。
由于该地形系统完全数据化生成，所以对寻路计算相比传统美术制作的底图更具有优势。同理在使用有限状态机或深度学习AI来制作NPC等角色时，程序会更加精简，模块化程度更高、灵活性更高。
在该地图系统基础之上，还可以拓展出战争迷雾、建筑物随机摆放、水体河流、超大地形拼接等功能，可拓展性非常高。
通过论文的工作，主要得出以下结论：
1. 正六边形地图系统相较于正方形地图系统，没有距离差异化，程序复杂度较低。
2. 通过程序化三角剖分生成的底图模型，对美术资源依赖度低，美术资源占用量小。
3. 对各类渲染管线兼容性好。在不同渲染管线中，程序模块保持不变，只需要替换相应的着色器，就可以达到相同活近似的视觉效果。
4. 可扩展性高。在现有地图系统基础上，可以很容易的添加寻路、战争迷雾、随机生成等功能。
随着软件和硬件的技术不断升级更新，越来越多新的技术与开发模式不断应用在游戏领域，对游戏的运行效率、硬件资源消耗、可玩性、视觉效果等诸多方面提出了新的挑战。地图系统作为一款游戏的核心模块，随着时代的进步在不断的更新。当今，随着“随机地下城”模式的流行，地图系统的灵活性、可扩展性提升到了一个新的高度。未来，在移动端游戏时长，会有越来越多的策略游戏借鉴PC端和主机端的游戏设计思路，在保持核心玩法不变的前提下，优化地图系统，涌现出更多操作简单、可玩性高的策略游戏。
...
致谢
紧张、充实而又难忘的大学学习生涯即将结束，在大学学习和撰写论文期间得到了许多人的帮助，使我终身难以忘怀。轻风系不住流云，流云却带走了岁月，打开尘封的记忆，往事如风却又历历在目，大学的学习生活即将结束。在这里我首先要感谢这四年来为我授课的各位老师，真心地说一句：你们辛苦了！感谢王洋老师在我的论文选题、定稿以及中期检查等方面都给了我精心的指导。您提出的宝贵意见使我在论文选题、撰写以及修改的过程中，不再像当初那样茫然无措，而是知道自己论文的不足和修改的方向。您正直、严谨的治学态度对我影响颇深，受益匪浅，无论在今后的学习还是工作当中，我都铭记于心。



translate-4071142-1590141311846
地图系统作为游戏中的一个重要组成部分，在许多RPG游戏、策略游戏、回合制游戏中是不可或缺的。地图系统随着硬件的不断更新换代，从最初的2D平面类型，如FC恶魔城、SFC塞尔达传说、GBA圣剑传说，由像素矩阵绘制而成的地图系统，经历了伪3D俯视角类型，如魔力宝贝、石器时代，由多种单一贴图所构成的地图系统，到现在非常流行的3D地图系统，如奇迹时代3、文明6、无尽传奇、英雄无敌，由材质、贴图、模型构成的地图系统。

Game map system is an important part of games, and is especially integral to many role-playing games, strategy games, and turn-based games. With the continuous iteration and upgrading of hardware and consoles, map systems have developed from what were 2D map systems, which were made up of pixel arrays, such as those in Family Computer’s Castlevania, Super Famicom’s The Legend of Zelda, and Gameboy Advance’s Sword of Mana, to the pseudo-3D top-down map systems, which were composed of multiple mono-maps, such as those in Devil Children and StoneAge, and again to the currently prevalent 3D map systems, which comprise of material textures, maps, and models, including those in Age of Wonders III, Civilization VI, Endless Legend, and Heroes of Might and Magic.



因此，地图系统的性能、存储容量、美观程度、灵活程度是一个游戏是否成功的重要因素。随着移动设备的不断普及，对游戏中地图系统的要求也在不断的提高。本文通过分析现有主流游戏的地图系统，结合了移动端快速生成、美术资源占用小、生成灵活、可数据化存储等要求，设计并开发了一个简单的可程序化生成与控制的地图系统。

Therefore, the performance, storage capacity, aesthetics and flexibility of map systems are crucial to the success of games. As the popularity of mobile devices continues to rise, the requirements for map systems in games are also growing. In this paper, by analyzing existing map systems of mainstream games, and enabling quick as well as flexible generation on the mobile terminal, limited footprint of art resources, and digitalized storage, the author designed and developed a simple map system that can be programmatically generated and controlled.
...
查阅相关资料，收集相关类型的游戏，使用Unity3D、Cocos2D、Unreal Engine引擎导入其地图美术资源，分析其地图系统的组成和实现方式。

1. Looking up relevant literature, collecting relevant games genres, importing their map art resources using Unity3D, Cocos2D and Unreal Engine, and analyzing the composition and implementation of their map systems.



综合比较同类游戏的地图系统，分析各自的优缺点。

2. Comparatively analyzing the pros and cons of the map systems in similar games.



根据分析结果创建基本的地图系统架构，并进行编码。

3. Creating and coding the basic architecture of map systems according to the analysis results.



反复试验，修改代码，得到最优结果。

4. Coming up with the optimized results through repeated testing and codes modifications.


Unity3D;Game engine;Map system;Architecture design